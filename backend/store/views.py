...
class ProductViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = Product.objects.filter(is_active=True).prefetch_related(
        "variants", "product_categories__category"
    )
    ...
    def get_queryset(self):
        ...
        if price_min or price_max:
            ...
            variant_ids = ProductVariant.objects.filter(variant_filter).values_list(
                "id", flat=True
            )
...
class AdminOrderViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def update_status(self, request, pk=None):
        ...
        log_admin_action(
            request.user, "update", "Order", str(order.id), {"status": new_status}
        )
...
def export_orders_csv(request):
    ...
    writer.writerow(["Reference", "Email", "Status", "Total", "Created At"])

    orders = Order.objects.all()
    for order in orders:
        writer.writerow(
            [order.reference, order.email, order.status, order.total, order.created_at]
        )

    log_admin_action(request.user, "export", "Order", "", {"count": orders.count()})
...
def client_update_profile(request):
    ...
    try:
        ...
        serializer = ClientUpdateSerializer(
            client_token.client, data=request.data, partial=True
        )
        if serializer.is_valid():
            serializer.save()
            return Response(
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            if not check_password(
                serializer.validated_data["current_password"], client.password
            ):
                return Response(
