...
from store.models import Baladiya, Wilaya
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if file_content is None:
                raise CommandError(
                    f"Could not read CSV file with any encoding: {csv_file}"
                )
            ...
            for field in required_fields:
                if field not in reader.fieldnames:
                    raise CommandError(
                        f'Required column "{field}" not found in CSV. Found columns: {reader.fieldnames}'
                    )
            ...
            for row in reader:
                ...
                if wilaya_name not in wilaya_cache:
                    wilaya, created = Wilaya.objects.get_or_create(
                        name=wilaya_name, defaults={"name": wilaya_name}
                    )
