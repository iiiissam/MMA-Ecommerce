...
class CategorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'image_url', 'is_active', 'children', 'product_count',
...
class ProductVariantSerializer(serializers.ModelSerializer):
    ...
    compare_at_price = serializers.DecimalField(max_digits=10, decimal_places=2, coerce_to_string=True, required=False, allow_null=True)

    class Meta:
        model = ProductVariant
        fields = [
            'id', 'product', 'sku', 'size', 'color', 'price', 'compare_at_price',
            'stock_quantity', 'low_stock_threshold', 'barcode', 'weight',
            'is_active', 'image_main', 'images', 'images_data', 'is_in_stock', 'is_low_stock',
            'created_at', 'updated_at'
        ]
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'is_active', 'variants', 'categories', 'min_price', 'max_price',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['slug', 'created_at', 'updated_at']
    ...
    def get_categories(self, obj):
        ...
        return [{'id': pc.category.id, 'name': pc.category.name, 'slug': pc.category.slug} for pc in categories]
...
class OrderLineSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OrderLine
        fields = [
            'id', 'product_variant', 'sku_snapshot', 'title_snapshot',
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [
            'id', 'reference', 'status', 'name', 'phone', 'address',
            'wilaya', 'wilaya_name', 'baladiya', 'baladiya_name',
            'subtotal', 'shipping_cost', 'total', 'payment_method',
            'payment_status', 'lines', 'created_at', 'updated_at'
        ]
    ...
    def validate_baladiya(self, value):
        ...
        try:
            ...
            if baladiya.wilaya.id != self.initial_data.get('wilaya'):
                raise serializers.ValidationError("Baladiya does not belong to selected wilaya.")
...
class AuditLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AuditLog
        fields = [
            'id', 'admin_user', 'admin_user_username', 'action_type',
            'model_name', 'object_id', 'changes', 'timestamp', 'ip_address'
        ]
...
class AdminProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'title', 'slug', 'description', 'brand',
            'is_active', 'variants', 'categories', 'created_at', 'updated_at'
        ]
        read_only_fields = ['slug', 'created_at', 'updated_at']
